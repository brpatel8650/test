...
const paths = {
    ...
    server: {
        ...
        mail : [`${serverPath}/mail/**/**/*.png`,`${serverPath}/mail/**/**/*.ejs`]
    },
    karma: 'karma.conf.js',
    dist: 'dist'
};
...
gulp.task('ejs', () => {
	    return gulp.src(paths.server.mail)
    		.pipe(gulp.dest(`${paths.dist}/${serverPath}/mail/`));
});
...
gulp.task('clean:translate', () => del(['client/components/translations/*'], {dot: true}));
gulp.task('translate:extract', () => {
    return gulp.src([paths.client.views, paths.client.scripts[0]])
        .pipe(gettext.extract('template.pot', {
            // options to pass to angular-gettext-tools...
        }))
        .pipe(gulp.dest('po/'));
});
// gulp.task('translate:extract-strings', () => {
//     // return gulp.src([paths.client.scripts[0]])
//     return gulp.src(['${clientPath}/app/account/signup/*.js', '${clientPath}/app/account/signup/*.html'])
//     .pipe(plugins.plumber())
//     .pipe(plugins.babel({ presets: ['es2015'] }))
//     // .pipe(gulp.src(['client/components/**/*.html']))
